// =====================================================================
// CytoFlow v1.3.7 AI Core - Multi-Database Architecture (SaaS Ready)
// =====================================================================
...
function doGet() {
  return HtmlService.createTemplateFromFile('index')
    .evaluate()
    .setTitle('CytoFlow 2026 (v1.3.7 AI Core)')
...
// --- API: GET MASTER DATA (ดึงจากหลายไฟล์ & ยุบรวม Reference เข้าด้วยกัน) ---
function apiGetMasterData() {
  try {
    ...
    // Helper function ดึงข้อมูลคอลัมน์ A อย่างเดียว
    var getColA = function(sheetName, targetSS) {
      ...
      for(var i=0; i<data.length; i++) {
        if(data[i][0] && data[i][0].toString().trim() !== "") {
          res.push(data[i][0].toString().trim());
        }
      }
      return res;
    };

    // Helper function ดึงข้อมูลคำนำหน้าและเพศ
    var getPrefixes = function() {
      ...
      for(var i=0; i<data.length; i++) {
        if(data[i][0] && data[i][0].toString().trim() !== "") {
          res.push({ 
            prefix: data[i][0].toString().trim(), 
            sex: (data[i][1] ? data[i][1].toString().trim() : 'หญิง') // Default เพศหญิง
          });
        }
      }
      return res;
...
function apiGetPatientHistory(cid, username) {
  ...
  try {
    ...
    // โหลดรายชื่อปีและ Spreadsheet ID ทั้งหมด
    const yearData = configSheet.getDataRange().getValues();
    
    // เริ่มลูปหาจากทุกๆ ปี (ข้ามแถวแรกที่เป็น Header)
    for (let i = 1; i < yearData.length; i++) {
      ...
      try {
        ...
        // โหลดข้อมูลทั้งชีทมาไว้ใน Memory เพื่อความรวดเร็วในการค้นหา
        ...
        for (let r = 0; r < data.length; r++) {
          ...
          if (rowCid === searchCid) {
            historyData.push({
              ...
              age: data[r][6],
              sex: data[r][7],
              specimenDate: formatDateVal(data[r][8]),
              recDate: formatDateVal(data[r][9]),
              unit: data[r][10] ? String(data[r][10]) : "ไม่ระบุ",
              district: data[r][11],
              hcode: data[r][12],
              coordinator: data[r][13],
              phone: data[r][14],
              para: data[r][15],
              last: data[r][16],
              lmp: formatDateVal(data[r][17]),
              contraception: data[r][18],
              prevTx: data[r][19],
              clinFind: data[r][20],
              clinDx: data[r][21],
              lastPap: data[r][22],
              method: data[r][23],
              registerName: data[r][24],
              regTimestamp: formatDateTimeVal(data[r][25]),
              adequacy: data[r][26],
              adequacyDetail: data[r][27],
              additional: data[r][28],
              organism: data[r][29] ? String(data[r][29]) : "",
              nonNeo: data[r][30] ? String(data[r][30]) : "",
              squamousMain: data[r][31] ? String(data[r][31]) : "",
              squamousSub: data[r][32] ? String(data[r][32]) : "",
              glandularMain: data[r][33] ? String(data[r][33]) : "",
              glandularSub: data[r][34] ? String(data[r][34]) : "",
              cat300: data[r][35],
              comment: data[r][36],
              cytoName: data[r][37],
              cytoDateTime: String(data[r][38]),
              pathoName: data[r][39],
              pathoDateTime: String(data[r][40]),
              status: data[r][41] ? String(data[r][41]) : "Pending"
            });
          }
        }
      } catch (err) {
    ...
    return { status: 'error', message: 'History System Error: ' + e.message };
  }
}
